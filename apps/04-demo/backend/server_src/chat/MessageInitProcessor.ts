...
export class MessageInitProcessor {
    ...
    async processMessage(userMessage: string): Promise<string> {
        try {
            ...
            const actionIdentification = await this.actionIdentifier.identifyActions(userMessage);
            ...
            if (actionIdentification.actions_required) {
                ...
                const actionsWithData: ActionWithData[] = actionIdentification.selected_actions.map(action => ({
                    type: action,
                    data: [messageData] // Default empty data array, can be populated with actual data if needed
                }));
                ...
                return await this.simpleHistoryBasedPrompt.generateResponse(userMessage);
            }
        } catch (error) {
