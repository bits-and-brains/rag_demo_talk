...
import { OpenAIProvider } from '../../llm/openai/OpenAIProvider';
...
export class GetRecipeAgent extends Agent {
    ...
    async execute(data?: ActionDataItem[]): Promise<void> {
        ...
        try {
            ...
            this.result = await this.searchRecipes(userMessage);
            ...
            if (this.result.length > 0) {
                ...
                this.response = await this.generateResponse(userMessage, this.recipeContent);
            } else {
     ...
     */
    private async searchRecipes(userMessage: string): Promise<string[]> {
        // Generate embedding for the user message
        const embedding = await this.openaiProvider.getEmbedding(userMessage, {
     ...
     */
    private async generateResponse(userMessage: string, recipeContent: string): Promise<string> {
        try {
